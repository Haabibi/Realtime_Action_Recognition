...
from multiprocess_streaming import streaming 
...
def _get_indices(data, style):
    ...
    tick =  (len(data) - new_length ) / float(args.test_segments)
...
if __name__=="__main__":
    ...
    parser.add_argument('-j', '--num_workers', default=5, type=int, metavar='N',
    ...
    for video in data_loader: 
        ...
        rgb_list, _tmp_of, of_list = list(), list(), Manager().list() 
        rst = 0
        num_frames = 0
        print(video)
        while(cap.isOpened()):
            ...
            if ret == True:
                ...
                _tmp_of.append(img)
                print("[LEN OF TMP OF]: ", len(_tmp_of))
                ...
                for i in range(args.num_workers):
                    for j in range( int(num_frames / args.num_workers) ) : 
                        pool.map(streaming, _tmp_of[args.num_workers*i+j], _tmp_of[args.num_workers*i+j + 1]) 
                print("LEN OF OF_LIST: ", len(of_list), type(of_list[0])) 
                
                #REST_NUM = num_frames /  
                #LAST_NUM = args.num_workers * int(num_frames / args.num_workers) 


                ''' 
                for i in range(int(num_frames/5)-1):
                    i = i*5 
                    proc = [ Process(target=streaming, args=(_tmp_of[i+0], _tmp_of[i+1], of_list, i)),
                             Process(target=streaming, args=(_tmp_of[i+1], _tmp_of[i+2], of_list, i+1)),
                             Process(target=streaming, args=(_tmp_of[i+2], _tmp_of[i+3], of_list, i+2)),
                             Process(target=streaming, args=(_tmp_of[i+3], _tmp_of[i+4], of_list, i+3)),
                             Process(target=streaming, args=(_tmp_of[i+4], _tmp_of[i+5], of_list, i+4)) ]
                    ...
                    print("LEN OF OF_LIST: ", len(of_list), type(of_list[0])) 
                '''
                REST_NUM = num_frames % 5  #num_frames = 162 .. REST = 2
                procs = []
                for i in range(REST_NUM-1):
                    procs.append( Process(target = streaming, args=(_tmp_of[num_frames-REST_NUM+i], _tmp_of[num_frames-REST_NUM+i+1], of_list, num_frames-REST_NUM+i)) )
                for p in procs:
                    p.start()
                ...
                inf_tic = time.time()
                rst = make_infer(args.of_weights, of_list, of_net, 'Flow')
                inf_toc = time.time()
                print("TIME for Processing: {}, TIME for INferencing : {}".format(inf_tic -process_start, inf_toc-inf_tic))
                ...
                print("RESULSTS: ", temp_rst, make_ucf()[temp_rst],  "[output length]: ", len(output), "how long it took to infer one video: ", inf_toc-inf_tic)
                break 
