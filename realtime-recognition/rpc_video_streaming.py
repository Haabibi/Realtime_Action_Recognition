    ...
    while True:
        ...
        while(cap.isOpened()):
            ...
            if ret == True:
                ...
                print("FRAME & SENDING: ", middle-beginning, end-middle)
            else:
                print("End of the streaming") 
                send_1 = time.time()
                send('End', ret)
                send_2 = time.time()
                print("END SENDING: ", send_2 - send_1)
                break
        event.is_set()
...
def receive_and_run_inference(rgb_net, queue):
    ...
    while True:
        ...
        if queue.qsize()>0:
            ...
            if len(frames_to_run) == 20:
                made_forty = time.time()
                print("MADE IT TON INF: ", made_forty-initial_time)
                rst = make_infer(args.rgb_weights, frames_to_run, rgb_net, 'RGB', args.test_segments, num_class)
                print("INFERENCE TIME: ", time.time() - made_forty)
