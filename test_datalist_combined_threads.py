...
def run_rgb_queue(rgb_queue, rgb_net, score_queue, in_progress):
    counter = 0
    vid_dir_file = open(args.vid_dir, 'r')
    num_lines = sum(1 for line in vid_dir_file)    
    while True:
        rgb_score = make_infer(rgb_queue.get(), rgb_net, 'RGB')  
        score_queue.put(rgb_score) 
        counter += 1
...
def run_of_queue(of_queue, of_net, score_queue, in_progress):
    ...
    while True:
        ...
        rgb_score = score_queue.get() 
        rgb_video_pred.append(np.argmax(np.mean(rgb_score, axis=0)))
        of_video_pred.append(np.argmax(np.mean(of_score, axis=0)))
        
        print("RGB: ", make_ucf()[np.argmax(np.mean(rgb_score, axis=0))]) 
        print("OF: ", make_ucf()[np.argmax(np.mean(of_score, axis=0))]) 
        
        score_fusion_time_1 = time.time()
        avg = (of_score*3 + rgb_score*2) / 5
        video_pred = np.argmax(np.mean(avg, axis=0))
        score_fusion_time_2 = time.time()
        fuse_video_pred.append(video_pred)
...
if __name__=="__main__":
    ...
    counter = 0 
    vid_dir_file = open(args.vid_dir, 'r')
    for line in vid_dir_file:
        vid_dir = line.split()[0]
        vid_label = line.split()[2]
    ...
    cf_rgb = confusion_matrix(video_labels, rgb_video_pred).astype(float)
    cf_of = confusion_matrix(video_labels, of_video_pred).astype(float)
    cf_fuse = confusion_matrix(video_labels, fuse_video_pred).astype(float)

    cls_cnt_rgb = cf_rgb.sum(axis=1)
    cls_cnt_of = cf_of.sum(axis=1)
    cls_cnt_fuse = cf_fuse.sum(axis=1)

    cls_hit_rgb = np.diag(cf_rgb)
    cls_hit_of = np.diag(cf_of)
    cls_hit_fuse = np.diag(cf_fuse)

    cls_acc_rgb = cls_hit_rgb / cls_cnt_rgb
    cls_acc_of = cls_hit_of / cls_cnt_of
    cls_acc_fuse = cls_hit_fuse / cls_cnt_fuse

    print('RGB Accuracy {:.02f}%'.format(np.mean(cls_acc_rgb) * 100))
    print('OF Accuracy {:.02f}%'.format(np.mean(cls_acc_of) * 100))
    print('FUSE Accuracy {:.02f}%'.format(np.mean(cls_acc_fuse) * 100))
