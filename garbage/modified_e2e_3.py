...
def get_from_queue(input_queue, output_queue, style):
    ...
    while True:
        ...
        if len(tmp_list) == frame_N:
            output_queue.put(tmp_list)
            toc = time.time()
            avg_extraction_time = avg_extraction_time / frame_N 
...
if __name__=="__main__":
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            toc1 = time.time()
            #frame_to_of_q.put(frame) 
            #toc2 = time.time()
            print("[F{}] checking q size".format(num_read_frames), frame_to_rgb_q.qsize(), frame_to_of_q.qsize())
            #if frame_to_of_q.qsize() % 500 == 1:
            #    print("streaming rgb took {:f}. streaming of took {:f}. Have {} frames in the rgb queue and {} many in of queue".format(toc1-tic, toc2-tic, frame_to_rgb_q.qsize(), frame_to_of_q.qsize()))
        else:
