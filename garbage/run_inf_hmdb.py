...
#output_txt = open("result_hmdbtrained_highquality.txt", "w+") 
output_txt = open(args.output_txt ,'w')
for video in sorted(os.listdir(args.root_path)):
 ...
 #       output_txt.write(video + '\t' + hmdb_datadict[video] + '\t' + index_dict[video_pred] + '\t' + str(toc-tic) + '\n') 
        #print(video, label, index_dict[video_pred]) 
        total_time += toc - tic
        print(video, index_dict[video_pred]) 
        #output_txt.write(video + '\t' + label + '\t' + index_dict[video_pred][:-1] + '\t' + str(toc-tic) + '\t' + str((toc-tic)/len(os.listdir(args.root_path))) + '\n')
        output_txt.write(video + '\t' + index_dict[video_pred][:-1] + '\t' + str(toc-tic) +  '\n')

output_txt.write(str((total_time)/len(os.listdir(args.root_path)))) 
'''
