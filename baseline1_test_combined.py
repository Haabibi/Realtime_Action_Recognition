...
if __name__=="__main__":
    ...
    video_data = open('/home/haabibi/fall_detection/tsn-pytorch/ucf101_file_lists/short_ucf_split.txt', 'r')
    ...
    for video in data_loader: 
        ...
        while(cap.isOpened()):
            ...
            if ret == True:
                ...
                inf_toc = time.time()
                print("video {} done, total {}/{}".format(counter, counter+1, len(data_loader) )) 
                counter+= 1
                output.append(of_rst)
                #print("[output length]: ", len(output), "how long it took to infer one video: ", inf_toc-inf_tic, "avg time streaming: ", accu_flow_toc/num_frames)
                break 
    video_pred = [np.argmax(np.mean(x[0], axis=0)) for x in output]
    print(video_pred)
    video_labels = [x[1] for x in data_loader]
